...
@Component
class UserDataResolver : ArgumentResolver {
    ...
    private fun resolveAccountData(accountData: AccountData): Mono<UserData> {
        return when (accountData) {
            is UserData -> createMono(accountData)
            is DeviceData -> createMonoError(UnAuthorizedException(SharedNexoraError.NEXORA0105))
        }
    }
}
