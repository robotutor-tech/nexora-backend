...
//    fun registerPremises(request: PremisesRequest, authUserData: AuthUserData): Mono<ActorDocument> {
...
//
//
//    private fun registerActor(authUserData: AuthUserData, request: PremisesRequest, roleDocuments: List<RoleDocument>): Mono<ActorDocument> {
//        val defaultHumanActorRoles = listOf(RoleType.USER, RoleType.ADMIN, RoleType.OWNER)
//        val humanRoles: List<RoleId> = roleDocuments.filter { defaultHumanActorRoles.contains(it.role) }.map { it.roleId }
...
//    }
//
//    private fun registerRoles(request: PremisesRequest, authUserData: AuthUserData): Flux<RoleDocument> {
...
//
//
//    private fun createInitialEntitlements(premisesId: PremisesId, roleDocuments: List<RoleDocument>): Flux<EntitlementDocument> {
//        val adminAndOwnerRoles = roleDocuments.filter { role -> role.role == RoleType.ADMIN || role.role == RoleType.OWNER }
...
//
//
//    private fun createReadPremisesEntitlement(premisesId: PremisesId, roleDocument: RoleDocument): Mono<EntitlementDocument> {
//        val entitlementRequest = EntitlementRequest(
...
//    }
//
//    fun registerDevice(request: RegisterDeviceRequest, invitationData: InvitationData): Mono<ActorDocument> {
