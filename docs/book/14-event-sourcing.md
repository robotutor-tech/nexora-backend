...
Traditional state-based storage loses history—each update overwrites the previous state, making audit trails, debugging, and compliance impossible. Event sourcing solves this by storing all changes as immutable events, enabling complete audit trails, time travel queries, and the ability to rebuild state at any point in time.

**What You'll Learn:**
- The lost history and compliance problem
- What event sourcing is and core principles
- Implementing an event store with MongoDB
- Rebuilding aggregate state from events
- Real device event sourcing from SmartHome Hub
- Snapshots for performance optimization
- Time travel queries to see historical state
- Event versioning and upcasting for evolution
...
1. The Problem: Lost History and No Audit Trail
...
In this chapter, we've explored Event Sourcing—storing all changes as a sequence of immutable events rather than just current state. This provides complete audit trails, time travel capabilities, and the ability to rebuild state at any point in history.

### What We Covered

**The Lost History Problem:**
- Traditional storage overwrites state
- No audit trail of changes
- Cannot prove historical states
- Compliance violations ($500K fine!)
...
device.status = "ACTIVE"  // Old status lost forever
repository.save(device)

// Event Sourcing (Event-Based) ✅
val event = DeviceActivated(deviceId, timestamp)
eventStore.append(deviceId, event)
// History preserved! Can rebuild anytime
```

### Key Insights

1. **Events are the source of truth** - Not current state, but history of changes.

2. **Current state is a projection** - Rebuild by replaying events.

3. **Events are immutable** - Once stored, never modified.

4. **Complete audit trail** - Know exactly what happened and when.

5. **Time travel queries** - See state at any point in history.

6. **Snapshots optimize performance** - Avoid replaying thousands of events.

7. **Optimistic concurrency** - Prevent lost updates with version checking.

8. **Event versioning essential** - Handle schema evolution over time.

9. **Debugging becomes trivial** - Replay events to reproduce issues.

10. **Compliance requirements met** - Full history for audits.
...
data class Device(
    @Id val id: String,
    val status: String,  // Current only
    val health: String,  // Current only
    val lastSeenAt: Long?  // Only last timestamp
)

// Update overwrites history
fun updateStatus(deviceId: String, newStatus: String) {
    val device = repository.findById(deviceId)
    device.status = newStatus  // Old status lost! ❌
    repository.save(device)
}

// Cannot answer: "Was device online on March 15?"
// Cannot prove: "Device was offline when customer claims"
// No audit trail for compliance
```

**After (Event Sourcing):**
```kotlin
// Events store complete history ✅
sealed class DeviceEvent(
    open val deviceId: DeviceId,
...
) : DeviceEvent(deviceId, timestamp, version)
...
// Event Store preserves everything
eventStore.append(deviceId, DeviceActivated(...))
eventStore.append(deviceId, DeviceWentOffline(...))

// Rebuild current state
val events = eventStore.loadEvents(deviceId)
val device = Device.fromEvents(events)

// Time travel: "Was device online on March 15?"
val historicalEvents = eventStore.loadEventsUntil(
    deviceId, 
    LocalDate.of(2024, 3, 15).atStartOfDay()
)
val historicalDevice = Device.fromEvents(historicalEvents)
println(historicalDevice.status)  // Can answer with certainty ✅
```

**Impact:**
- Complete audit trail for compliance
- Time travel queries answer historical questions
- Full debugging capability
- Never lose data
- Compliance fine avoided ($500K saved!)
...
interface EventStore {
    ...
    fun loadEvents(streamId: StreamId): List<DomainEvent>
...
class MongoEventStore(
    ...
    override fun append(
        ...
        events.forEachIndexed { index, event ->
            ...
            mongoTemplate.save(storedEvent)
        }
    }
    
    override fun loadEvents(streamId: StreamId): List<DomainEvent> {
        val query = Query.query(Criteria.where("streamId").`is`(streamId.value))
            .with(Sort.by(Sort.Direction.ASC, "version"))
        
        return mongoTemplate.find(query, StoredEvent::class.java)
...
class Device private constructor(
    ...
    companion object {
        ...
        fun fromEvents(events: List<DeviceEvent>): Device {
            require(events.isNotEmpty()) { "No events to rebuild from" }
            
            val first = events.first() as DeviceRegistered
            var device = Device(
    ...
    private fun apply(event: DeviceEvent): Device {
        return when (event) {
            is DeviceActivated -> copy(
                status = DeviceStatus.ACTIVE,
                version = event.version
            )
            is DeviceDeactivated -> copy(
                status = DeviceStatus.INACTIVE,
                version = event.version
            )
            is DeviceWentOffline -> copy(
                health = DeviceHealth.OFFLINE,
                version = event.version
            )
            is DeviceWentOnline -> copy(
    ...
    fun activate(): List<DeviceEvent> {
        require(status == DeviceStatus.PENDING) { 
            "Can only activate pending devices" 
        }
        return listOf(DeviceActivated(deviceId, Instant.now(), version + 1))
    }
}
```
...
**Without Snapshots:**
```kotlin
// Load 10,000 events and replay all ❌
val events = eventStore.loadEvents(deviceId)  // Slow!
val device = Device.fromEvents(events)
```
...
val eventsSinceSnapshot = eventStore.loadEventsAfter(
    deviceId, 
    snapshot.version
)
val device = Device.fromSnapshot(snapshot)
    .applyEvents(eventsSinceSnapshot)

// Much faster! Replay only recent events
```
...
interface SnapshotStore {
    fun save(snapshot: DeviceSnapshot)
    fun loadLatest(deviceId: DeviceId): DeviceSnapshot?
}

// Save snapshot every 100 events
fun saveDeviceIfNeeded(device: Device) {
...
```kotlin
// "What was device status on March 15, 2024?"
fun getDeviceStateAt(
    deviceId: DeviceId, 
    date: LocalDate
): Device {
    val timestamp = date.atStartOfDay().toInstant(ZoneOffset.UTC)
    val events = eventStore.loadEventsUntil(deviceId, timestamp)
    return Device.fromEvents(events)
}

// Usage
val device = getDeviceStateAt(
    deviceId = DeviceId("device-123"),
    date = LocalDate.of(2024, 3, 15)
)
println("Status on March 15: ${device.status}")

// "When did device go offline?"
fun findFirstOfflineEvent(deviceId: DeviceId): Instant? {
    return eventStore.loadEvents(deviceId)
        .filterIsInstance<DeviceWentOffline>()
        .firstOrNull()
        ?.timestamp
}

// "How many times did device go offline in March?"
fun countOfflineEvents(
    deviceId: DeviceId, 
    month: YearMonth
): Int {
    val start = month.atDay(1).atStartOfDay().toInstant(ZoneOffset.UTC)
    val end = month.atEndOfMonth().atTime(23, 59, 59)
        .toInstant(ZoneOffset.UTC)
    
    return eventStore.loadEvents(deviceId)
        .filterIsInstance<DeviceWentOffline>()
        .count { it.timestamp.isAfter(start) && it.timestamp.isBefore(end) }
}
```
...
interface EventUpcaster {
    fun upcast(event: Any): Any
}

class DeviceActivatedUpcaster : EventUpcaster {
    override fun upcast(event: Any): Any {
        return when (event) {
            is DeviceActivatedV1 -> DeviceActivatedV2(
                deviceId = event.deviceId,
                activatedBy = "SYSTEM",  // Default for old events
                timestamp = event.timestamp,
                version = event.version
            )
            else -> event
...
// Apply upcasters when loading
fun loadEventsWithUpcast(streamId: StreamId): List<DomainEvent> {
    val storedEvents = mongoTemplate.find(...)
    return storedEvents.map { stored ->
        val event = deserializeEvent(stored)
        upcasters.fold(event) { e, upcaster -> upcaster.upcast(e) }
    }
}
```
...
```kotlin
@Test
fun `should activate device when pending`() {
    // Given - prior events
    val givenEvents = listOf(
        DeviceRegistered(deviceId, serialNumber, timestamp, 1L)
    )
    val device = Device.fromEvents(givenEvents)
    
    // When - command
    val newEvents = device.activate()
    
    // Then - new events produced
    assertEquals(1, newEvents.size)
    assertTrue(newEvents.first() is DeviceActivated)
    assertEquals(2L, newEvents.first().version)
}
...
fun `should not activate already active device`() {
    val givenEvents = listOf(
        DeviceRegistered(deviceId, serialNumber, timestamp, 1L),
        DeviceActivated(deviceId, timestamp, 2L)
    )
    val device = Device.fromEvents(givenEvents)
...
```kotlin
@Test
fun `should store and load events in order`() {
    val events = listOf(
        DeviceRegistered(deviceId, serialNumber, timestamp, 1L),
        DeviceActivated(deviceId, timestamp.plusSeconds(60), 2L)
    )
    
    eventStore.append(deviceId, events)
    
    val loadedEvents = eventStore.loadEvents(deviceId)
    
    assertEquals(2, loadedEvents.size)
    assertTrue(loadedEvents[0] is DeviceRegistered)
    assertTrue(loadedEvents[1] is DeviceActivated)
}

@Test
fun `should detect concurrency conflict`() {
    eventStore.append(deviceId, listOf(DeviceRegistered(...)), null)
    
    // Expected version 1, but current is 1 (someone else wrote)
    assertThrows<ConcurrencyException> {
        eventStore.append(deviceId, listOf(DeviceActivated(...)), 0L)
    }
}
```
...
Teams implementing event sourcing see:
- **100% audit trail** coverage for compliance
- **Complete debugging** capability (replay to reproduce)
- **Time travel queries** answer historical questions
- **No data loss** - everything preserved
- **Better insights** - analyze event patterns
- **Regulatory compliance** achieved

### Practice Exercise

Add event sourcing to your aggregate:

1. **Choose aggregate** - Start with most critical (e.g., Order, Account)
2. **Design events** - All state changes as events
3. **Implement event store** - MongoDB or PostgreSQL
4. **Create event-sourced aggregate** - Apply methods for each event
5. **Add repository** - Load events, rebuild state, append new
6. **Implement snapshots** - Every 100 events
7. **Add time travel query** - Historical state lookup
8. **Test thoroughly** - Given/when/then with events
9. **Monitor performance** - Snapshot effectiveness

### Design Checklist

When implementing event sourcing:
- ✅ Events are immutable
- ✅ Events in past tense (DeviceActivated, not ActivateDevice)
- ✅ Complete information in events
- ✅ Optimistic concurrency with versioning
- ✅ Snapshots for performance (every N events)
- ✅ Event upcasting for version evolution
- ✅ Time travel query support
- ✅ Audit trail accessible
- ✅ Event store tested for concurrency
- ✅ Monitoring for event store performance

---

### Additional Reading

For deeper understanding of event sourcing:
- **"Versioning in an Event Sourced System"** by Greg Young
- **"Event Sourcing"** by Martin Fowler - Comprehensive overview
- **"Implementing Domain-Driven Design"** by Vaughn Vernon (2013) - Event sourcing chapters

---

## What's Next

In **Chapter 15**, we'll explore the Builder Pattern for constructing complex aggregates with many optional parameters. You'll learn:
- When builders simplify object creation
- Fluent builder API design
- Director pattern for common configurations
- Validation in builders
- Testing with builders
- Real aggregate builders from SmartHome Hub

With event sourcing providing complete history, builders will help create complex objects cleanly.

Turn the page to master the Builder Pattern...
